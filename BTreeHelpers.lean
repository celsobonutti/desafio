...
def keysWithPrefix (tree : BTree) (prefix : String) : IO (Array String) :=
  tree.filter prefix

/-- Get all key-value pairs with a given prefix -/
def pairsWithPrefix (tree : BTree) (prefix : String) : IO (Array (String × ByteArray)) := do
  let keys ← tree.filter prefix
  let mut pairs := #[]
  for key in keys do
    if let some value ← tree.read key then
